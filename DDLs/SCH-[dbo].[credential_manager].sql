...
begin
	...
	(server_ip, server_name, [user_name], password_hash, salt, is_sql_user, is_rdp_user, remarks)
	select server_ip = @server_ip, server_name = @server_name, [user_name] = @user_name,
			...
			is_sql_user = @is_sql_user, is_rdp_user = @is_rdp_user, remarks = @remarks;
...
as
begin
	if @server_ip is null and @user_name is null and @server_name is null
		throw 50000, 'Kindly provide both server_ip/server_name or user_name.', 1;

	if IS_SRVROLEMEMBER('SYSADMIN') <> 1
		print 'Since caller is not a sysadmin, Only look for credentials created/updated by caller.'
	...
	and (	IS_SRVROLEMEMBER('SYSADMIN') = 1
		or
			(created_by = SUSER_NAME() or updated_by = SUSER_NAME())
		);
...
-- Check all Logins
select server_ip, server_name, [user_name], is_sql_user, is_rdp_user, 
		password_hash, salt, created_date, created_by, updated_date, updated_by, remarks 
from dbo.credential_manager
go

-- Fetch password
declare @password varchar(256);
exec DBA_Inventory.dbo.usp_get_credential 
		--@server_ip = '*',
		--@user_name = 'Lab\SQLServices',
