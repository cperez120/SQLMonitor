...
IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_NAME = 'usp_rebuilt_resource_consumption')
    EXEC ('CREATE PROC dbo.usp_rebuilt_resource_consumption AS SELECT ''stub version, to be replaced''')
GO

ALTER PROCEDURE dbo.usp_rebuilt_resource_consumption
	@verbose tinyint = 0,
	@dry_run bit = 0
AS 
BEGIN
	...
	/*
		...
		EXEC dbo.usp_rebuilt_resource_consumption @verbose = 2, @dry_run = 1;
	*/
	SET NOCOUNT ON;
	...
	declare @TableName nvarchar(125) = 'dbo.resource_consumption';
	...
	if exists (select 1 from sys.indexes i where i.object_id = OBJECT_ID('dbo.resource_consumption') and i.index_id <= 1 and i.type_desc = 'CLUSTERED COLUMNSTORE')
		set @_is_columnstore = 1;

	if @_is_columnstore = 0
		print '('+convert(varchar,getdate(),120)+')'+@_tab+'Table [dbo].[resource_consumption] is not clustered columnstore table.';
	else
		print '('+convert(varchar,getdate(),120)+')'+@_tab+'Table [dbo].[resource_consumption] qualify clustered columnstore table scenario.'

	if object_id('dbo.resource_consumption_staging') is null and @_is_columnstore = 1
	begin
		if @verbose > 0
			print '('+convert(varchar,getdate(),120)+')'+@_tab+'Table [dbo].[resource_consumption_staging] does not exist. So creating it..'
		CREATE TABLE [dbo].[resource_consumption_staging]
		(
	...
	begin
		if @verbose > 0
			print '('+convert(varchar,getdate(),120)+')'+@_tab+'Table [dbo].[resource_consumption_staging] already exists.'
	end

	if not exists (select 1 from dbo.resource_consumption) and @_is_columnstore = 1
	begin
		if @verbose > 0
			print '('+convert(varchar,getdate(),120)+')'+@_tab+'Table [dbo].[resource_consumption_staging] already exists.'
		...
		WHERE 1=1
		and (i.object_id = OBJECT_ID('dbo.resource_consumption'))
