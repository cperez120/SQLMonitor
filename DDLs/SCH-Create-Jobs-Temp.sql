...
/****** Object:  Job [(dba) Partitions-Maintenance]    Script Date: Wed, 30 Mar 22:37:15 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [(dba) Monitoring & Alerting]    Script Date: Wed, 30 Mar 22:37:15 ******/
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Add partitions - Hourly - Till Next Quarter End]    Script Date: Wed, 30 Mar 22:37:15 ******/
...
begin
	...
	alter partition function pf_dba() split range (@current_boundary_value);	
		...
		@flags=12
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Remove Partitions - Retain upto 3 Months]    Script Date: Wed, 30 Mar 22:37:15 ******/
...
/****** Object:  Job [(dba) Purge-DbaMetrics - Daily]    Script Date: Wed, 30 Mar 22:35:47 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [(dba) Monitoring & Alerting]    Script Date: Wed, 30 Mar 22:35:47 ******/
IF NOT EXISTS (SELECT name FROM msdb.dbo.syscategories WHERE name=N'(dba) Monitoring & Alerting' AND category_class=1)
BEGIN
EXEC @ReturnCode = msdb.dbo.sp_add_category @class=N'JOB', @type=N'LOCAL', @name=N'(dba) Monitoring & Alerting'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
dbo.perfmon_files', 
		...
		@notify_email_operator_name=N'SQLAgentService', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [dbo.performance_counters]    Script Date: Wed, 30 Mar 22:35:47 ******/
...
begin
	...
	set @r = @@ROWCOUNT
		...
		@flags=12
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [dbo.perfmon_files]    Script Date: Wed, 30 Mar 22:35:47 ******/
...
begin
	...
	set @r = @@ROWCOUNT
		...
		@flags=12
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [dbo.os_task_list]    Script Date: Wed, 30 Mar 22:35:47 ******/
