...
IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_NAME = 'usp_collect_xevents_resource_consumption')
    EXEC ('CREATE PROC dbo.usp_collect_xevents_resource_consumption AS SELECT ''stub version, to be replaced''')
GO

ALTER PROCEDURE dbo.usp_collect_xevents_resource_consumption
AS 
BEGIN
	/*
		Version:		1.1.5
