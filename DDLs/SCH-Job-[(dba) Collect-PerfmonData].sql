...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Import-PerfmonData', 
		...
		@command=N'powershell.exe -executionpolicy bypass -Noninteractive  E:\Perfmon\perfmon-collector-push-to-sqlserver.ps1 -HostName YourHostName -SqlInstance YourSqlInstanceName -Database DBA', 
		@flags=40
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Import-TaskList', 
		...
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'CmdExec', 
		@command=N'powershell.exe -executionpolicy bypass -Noninteractive  E:\Perfmon\tasklist-push-to-sqlserver.ps1 -HostName YourHostName -SqlInstance YourSqlInstanceName -Database DBA', 
		@flags=40
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
