...
t_queries as (
	...
	where w.collection_time >= dateadd(day,-15,getdate()) and w.collection_time <= getdate()
	and w.database_name = 'SOS'
	and (	convert(varchar(max),w.sql_text) like '%[[. ]Stgtbl_ClientDetails_Broking[!] ]%' escape '!' )
...
go
/*
select top 1000 sql_text +char(10)+'GO'
from dbo.resource_consumption rc
where rc.event_time >= dateadd(day,-7,getdate()) and event_time <= getdate()
and rc.database_name = 'remisior'
and (	convert(varchar(max),rc.sql_text) like '%[. ]tbl_ebrok_detail_cli %' escape '!' or convert(varchar(max),rc.sql_text) like '%![tbl_ebrok_detail_cli!]%' escape '!' )
*/