...
select @schedulers = count(*), @cpu_time_min = sum(total_cpu_usage_ms/1000.0)/60.0 from sys.dm_os_schedulers where status = 'VISIBLE ONLINE' and is_online = 1;
...
SELECT
	...
	CONVERT(DECIMAL(18,1), MAX([W1].[SignalS]) * 100.0 / MAX([W1].[WaitS]) ) AS [Percent Signal Waits],
	MAX([W1].[WaitCount]) AS [Number of Waits],
	CONVERT(DECIMAL(18,1), MAX([W1].[wait_time_ms]) / MAX([W1].[WaitCount]) ) AS [Avg ms Per Wait],
